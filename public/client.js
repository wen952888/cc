...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function handleAuthSuccess(data) {
        if (!data || !data.userId) {
            console.error("[AUTH] handleAuthSuccess called with invalid data:", data);
            showAuthError("认证数据无效，请重试。");
            switchToView('auth-view');
            return;
        }
        ...
        clearAuthError(); console.log(`[AUTH] Auth success for user: ${myUsername} (ID: ${myUserId})`);

        if (data.roomState && data.roomState.roomId) {
            currentRoomId = data.roomState.roomId;
            console.log(`[AUTH] User was in room ${currentRoomId}, displaying game state.`);
            displayGameState(data.roomState, true); switchToView('game-view');
        } else {
            if (currentRoomId) { currentRoomId = null; currentRoomState = null; }
            console.log('[AUTH] User not in a room, switching to lobby.');
            switchToView('lobby-view'); 
            if (socket.connected) socket.emit('listRooms', updateRoomList); else console.warn("[AUTH] Socket not connected, cannot fetch room list yet.");
        }
    }
    function handleAuthResponse(response) {
        console.log('[AUTH] Login/Re-auth response received:', response);
        if (response && response.success) handleAuthSuccess(response);
        else { 
    ...
    if (readyButton) {
        readyButton.addEventListener('click', () => {
            if (!currentRoomState || !myUserId) { console.warn("[READY] No room state or user ID."); return; }
            const myPlayer = currentRoomState.players.find(p => p.userId === myUserId);
            if (!myPlayer || currentRoomState.status !== 'waiting') { console.warn("[READY] Not in waiting state or player not found."); return; }
    ...
    function displayGameState(state, animateHandOnDisplay = false) {
        ...
        if (myPlayer) {
            updatePlayerHandUI(myPlayer.hand || [], state.status === 'playing' && state.currentPlayerId === myPlayer.userId && myPlayer.connected && !myPlayer.finished, animateHandOnDisplay);
            const isWaiting = state.status === 'waiting'; const isPlaying = state.status === 'playing';
            if(readyButton) { readyButton.style.display = isWaiting && !state.gameFinished ? 'inline-block' : 'none'; readyButton.disabled = state.gameFinished; readyButton.textContent = myPlayer.isReady ? "取消" : "准备"; readyButton.classList.toggle('cancel-ready', myPlayer.isReady); }
            const showGameplayButtons = isPlaying && myPlayer.connected && !myPlayer.finished;
            [hintButton, passButton, playButton].forEach(btn => { if(btn) btn.style.display = showGameplayButtons ? 'inline-block' : 'none'; });
            if (playButton) playButton.disabled = !(showGameplayButtons && state.currentPlayerId === myPlayer.userId);
            if (passButton) passButton.disabled = !(showGameplayButtons && state.currentPlayerId === myPlayer.userId && !state.isFirstTurn && state.lastHandInfo && (!state.lastPlayerWhoPlayedId || state.lastPlayerWhoPlayedId !== myUserId));
            if (hintButton) hintButton.disabled = !(showGameplayButtons && state.currentPlayerId === myPlayer.userId);
