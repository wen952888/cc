...
function setupRemoteAudioPlayer(userId) {
    ...
    remoteAudioPlayers[userId] = {
        audio: audio, mediaSource: mediaSource, sourceBuffer: null,
        queue: bufferQueue, processQueue: processBufferQueue, speaking: false
    };
...
function handleRemoteAudioChunk(userId, chunk) {
    ...
    if (!player) player = setupRemoteAudioPlayer(userId);
...
function setVoiceIndicator(userId, isSpeaking) {
    ...
    if (remoteAudioPlayers[userId]) remoteAudioPlayers[userId].speaking = isSpeaking;
}
    ...
    if (actionsContainers.length > 0) {
        ...
        if (isMyTurnAndCanPlay) {
            ...
            if(passTurnButton) {
                ...
                if (state.isFirstTurn && myPlayerInState.hand && myPlayerInState.hand.some(c=>c.rank==='4'&&c.suit==='D')) disablePass = true;
...
function renderRoomView(state) {
    ...
    if (myHandContainer) myHandContainer.innerHTML = ''; else console.error("[DEBUG] #myHand 容器未找到，无法清空!");
    updateGameInfoBarDOM(state);
    updateGameStatusDisplayDOM(state);
    Object.values(playerAreas).forEach(area => {
        if (area.id !== 'playerAreaBottom') clearPlayerAreaDOM(area);
        else {
    ...
    if (!isMyTurnAndCanPlayNow) clearHintsAndSelection(true);
}
...
function clearPlayerAreaDOM(area) {
    ...
    if (infoEl) infoEl.innerHTML = '总分: 0';
    if (voiceIndicator) voiceIndicator.classList.remove('speaking');
    if (cardsEl && area.id !== 'playerAreaBottom') cardsEl.innerHTML = '<span style="color:#888; font-style:italic;">- 等待 -</span>';
...
function renderPlayerArea(container, playerData, isMe, state, absoluteSlot) {
    ...
    if (!playerData || !playerData.userId) { clearPlayerAreaDOM(container); return; }
    if (avatarEl) {
        avatarEl.innerHTML = '';
        ...
        if (state.status === 'playing' && playerData.userId === state.currentPlayerId && !playerData.finished) {
            ...
            alarmImg.src = ALARM_ICON_SRC; alarmImg.alt = '出牌提示'; alarmImg.classList.add('alarm-icon');
            avatarEl.appendChild(alarmImg); avatarEl.style.backgroundImage = 'none';
        }
    }
    ...
    if (infoEl) {
        ...
        if (state.status === 'waiting' && !isMe) infoText += playerData.isReady ? ' <span class="ready">[已准备]</span>' : ' <span class="not-ready">[未准备]</span>';
        else if (playerData.finished) infoText += ' <span class="finished">[已完成]</span>';
...
function renderPlayerCards(containerParam, playerData, isMe, isMyTurnAndCanPlay) {
    ...
    if (playerData && Array.isArray(playerData.hand)) handToRender = [...playerData.hand];
        ...
        targetContainer.innerHTML = `<span style="color:#cc0000; font-style:italic;">手牌同步中 (${playerData.handCount} 张)...</span>`; return;
    } else if (playerData && !playerData.finished) console.warn(`[RenderPlayerCards] 我的手牌不是数组 (用户: ${playerData.username}, 完成状态: ${playerData.finished}). 渲染为空.`);
    ...
    if (playerData.finished) targetContainer.innerHTML = '<span style="color:#888; font-style:italic;">已出完</span>';
        ...
        handToRender.forEach((cardData, index) => {
            ...
            cardElement.className = 'card visible'; cardElement.style.transform = ''; cardElement.style.zIndex = index;
            if (isMyTurnAndCanPlay) {
                ...
                else if (isHinted) cardElement.classList.add('hinted');
...
function clearHintsAndSelection(resetSelectionAndCycle = true) {
    if (resetSelectionAndCycle) { currentHint = null; currentHintCycleIndex = 0; selectedCards = []; if(playSelectedCardsButton) playSelectedCardsButton.disabled = true; }
    const localMyHandArea = document.getElementById('myHand');
    if (localMyHandArea) {
        const cardElements = localMyHandArea.querySelectorAll('.card');
        cardElements.forEach(el => el.classList.remove('hinted', 'selected'));
    }
}
...
function toggleCardSelection(cardData, cardElement) {
    ...
    if (cardElement.classList.contains('hinted') && cardElement.classList.contains('selected')) cardElement.classList.remove('hinted');
    else if (currentHint && currentHint.cards.some(c => c.rank === cardData.rank && c.suit === cardData.suit) && !cardElement.classList.contains('selected')) cardElement.classList.add('hinted');
    if (playSelectedCardsButton && currentGameState && currentGameState.currentPlayerId === myUserId) playSelectedCardsButton.disabled = selectedCards.length === 0;
}
function handlePlaySelectedCards() {
    ...
    socket.emit('playCard', selectedCards, (response) => {
        if (!response.success) {
            ...
            if (currentGameState && currentGameState.status === 'playing' && currentGameState.currentPlayerId === myUserId) { setGameActionButtonsDisabled(false); updateRoomControls(currentGameState); }
        } else {
            if (currentGameState) {
                const myPlayer = currentGameState.players.find(p => p.userId === myUserId);
...
function showGameOver(resultData) {
    ...
    if (playersToDisplay.length > 0) {
        playersToDisplay.forEach(playerData => {
            ...
            p.innerHTML = scoreText; gameOverScores.appendChild(p);
        });
    } else gameOverScores.innerHTML = '<p>无法加载得分详情。</p>';
...
socket.on('playerLeft', ({ userId, username, reason }) => {
    if (currentGameState && currentView === 'roomView') {
        ...
        setVoiceIndicator(userId, false); renderRoomView(currentGameState);
...
socket.on('gameStarted', (initialGameState) => { if (currentView !== 'roomView' || currentRoomId !== initialGameState.roomId) return; currentGameState = initialGameState; if (gameStatusDisplay) displayMessage(gameStatusDisplay, '游戏开始！祝你好运！', false, true); selectedCards = []; clearHintsAndSelection(true); renderRoomView(currentGameState); });
socket.on('gameStateUpdate', (newState) => {
    ...
    if (myNewPlayerState) {
        if (Array.isArray(myNewPlayerState.hand)) { /* Server sent full hand */ }
        else if (myNewPlayerState.finished || myNewPlayerState.handCount === 0) myNewPlayerState.hand = [];
        else if (myNewPlayerState.hand === undefined) {
            if (myOldHand && myOldPlayerState && !myNewPlayerState.finished) myNewPlayerState.hand = myOldHand;
            else if (myNewPlayerState.handCount > 0 && !myNewPlayerState.finished) {
                console.warn("[gameStateUpdate] Hand count > 0, but hand array is undefined. Requesting full state.");
                socket.emit('requestGameState', (fullState) => { if(fullState) { currentGameState = fullState; renderRoomView(currentGameState); }}); return;
    ...
    if (previousGameState && currentGameState) {
      ...
      if (myTurnChangedToNotMyTurn || newRoundStartedForMe || iPlayedLastAndNowNewRound) { selectedCards = []; clearHintsAndSelection(true); }
    }
...
socket.on('audioChunk', ({ userId, username, chunk }) => { if (userId !== myUserId && currentView === 'roomView') handleRemoteAudioChunk(userId, chunk); });
socket.on('playerStartedSpeaking', ({ userId, username }) => { if (userId !== myUserId && currentView === 'roomView') { setVoiceIndicator(userId, true); if (remoteAudioPlayers[userId]) remoteAudioPlayers[userId].speaking = true; console.log(`${username} 开始说话`); } });
socket.on('playerStoppedSpeaking', ({ userId }) => { if (userId !== myUserId && currentView === 'roomView') { setVoiceIndicator(userId, false); if (remoteAudioPlayers[userId]) remoteAudioPlayers[userId].speaking = false; console.log(`玩家 ${userId} 停止说话`); } else if (userId === myUserId && !isMicActive) setVoiceIndicator(myUserId, false); });
        ...
        socket.emit('reauthenticate', storedUserId, (response) => {
            if (response.success) {
                ...
                if (response.roomState) {
                    currentRoomId = response.roomState.roomId; currentGameState = response.roomState;
                    if (currentGameState.status === 'finished') { showView('roomView'); renderRoomView(currentGameState); showGameOver(currentGameState); }
                    else { showView('roomView'); renderRoomView(currentGameState); initializeAudioFeatures(); }
                } else showView('lobbyView');
            } else {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (backToLobbyButton) backToLobbyButton.addEventListener('click', () => {
        if (currentRoomId) {
            ...
            if (actualLeaveButton && !actualLeaveButton.disabled) handleGameLeave(); else handleReturnToLobby();
