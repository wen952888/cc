...
document.addEventListener('DOMContentLoaded', () => {
    console.log('DOM fully loaded and parsed. Client v1.0.35'); // 版本号更新
    const socket = io({
    ...
    if (aiToggleButton) {
        aiToggleButton.addEventListener('click', () => {
            if (!currentRoomState || !myUserId) { alert("无法切换AI状态：不在房间内或未登录。"); return; }
            const myPlayer = currentRoomState.players.find(p => p.userId === myUserId);
            if (!myPlayer) { alert("无法切换AI状态：找不到玩家信息。"); return; }
            const newAiState = !isAi托管激活; 
            ...
            socket.emit('toggleAI', { enabled: newAiState }, (response) => {
                if (response && response.success) {
                    ...
                    if (response.message) { showTemporaryMessage(response.message); }
                } else {
    ...
    function showTemporaryMessage(message, duration = 2500) { /* ... (保持不变) ... */ 
        const toast = document.createElement('div'); toast.textContent = message; toast.style.position = 'fixed'; toast.style.bottom = '70px';  toast.style.left = '50%'; toast.style.transform = 'translateX(-50%)'; toast.style.backgroundColor = 'rgba(0,0,0,0.75)'; toast.style.color = 'white'; toast.style.padding = '10px 15px'; toast.style.borderRadius = '5px'; toast.style.zIndex = '2000'; toast.style.fontSize = '0.9em'; document.body.appendChild(toast); setTimeout(() => { toast.remove(); }, duration);
    }
    ...
    // Leave Room, Game Action Buttons 
    ...
    function displayGameState(state, animateHandOnDisplay = false) {
        ...
        if (myInfoInBar && myPlayer) { 
            ...
            if (myReadyEl) {
                ...
                myReadyEl.style.display = state.status === 'waiting' && !state.gameFinished ? 'inline-block' : 'none';
            }
            ...
            const myStatusEl = myInfoInBar.querySelector('#myPlayerStatus .card-count');
            const myReadyEl = myInfoInBar.querySelector('.player-ready-status');
            if(myNameEl) myNameEl.textContent = "玩家?";
            if(myStatusEl) myStatusEl.textContent = "?";
            if(myReadyEl) myReadyEl.style.display = 'none';
        } else {
            console.warn("[DISPLAY] myInfoInBar element not found!"); // 添加警告
        }

        const opponentSlotMap = {};
        if (myPlayer && state.players && state.players.length > 0) { const mySlot = myPlayer.slot; const numPlayers = state.players.length > 1 ? state.players.length : 4;  const relativeSlots = { top: (mySlot + 2) % numPlayers, left: (mySlot + 3) % numPlayers, right: (mySlot + 1) % numPlayers }; if (numPlayers === 2) {  relativeSlots.left = -1; relativeSlots.right = -1;  relativeSlots.top = (mySlot + 1) % numPlayers; } else if (numPlayers === 3) {  relativeSlots.top = -1;  relativeSlots.left = (mySlot + 2) % numPlayers; relativeSlots.right = (mySlot + 1) % numPlayers; } for (const posKey in relativeSlots) { if (relativeSlots[posKey] !== -1) { opponentSlotMap[posKey] = state.players.find(p => p.slot === relativeSlots[posKey] && p.userId !== myUserId); } } } else if (state.players) { const otherPlayers = state.players.filter(p => p.userId !== myUserId); if (otherPlayers[0]) opponentSlotMap['top'] = otherPlayers[0]; if (otherPlayers[1]) opponentSlotMap['left'] = otherPlayers[1]; if (otherPlayers[2]) opponentSlotMap['right'] = otherPlayers[2]; }
        ...
        if (myPlayer) {
            ...
            if (aiToggleButton) aiToggleButton.disabled = state.gameFinished || !myPlayer.connected || state.status !== 'playing'; 
